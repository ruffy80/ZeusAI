...
(function(){
  ...
  function coverFor(slug){var s=String(slug||"product").toLowerCase().replace(/[^a-z0-9-]+/g,"-").replace(/^-+|-+$/g,"")||"product";return "/api/dropship/cover/"+encodeURIComponent(s)+".svg";}
  function safeImage(url,slug){url=String(url||"").trim();if(/^https?:\/\//i.test(url)||(url.charAt(0)==="/"&&url.indexOf("/api/dropship/")===0))return esc(url);return coverFor(slug);}

...
(function(){
  ...
  function esc(s){return String(s==null?"":s).replace(/[&<>"]/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c];});}
  function money(n){return "$"+Number(n||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2});}
  function safeImage(url,slug){url=String(url||"").trim();if(/^https?:\/\//i.test(url)||(url.charAt(0)==="/"&&url.indexOf("/api/dropship/")===0))return esc(url);var s=String(slug||"product").toLowerCase().replace(/[^a-z0-9-]+/g,"-").replace(/^-+|-+$/g,"")||"product";return "/api/dropship/cover/"+encodeURIComponent(s)+".svg";}

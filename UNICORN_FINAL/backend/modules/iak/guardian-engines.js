...
class UnicornOrchestrator extends EventEmitter {
  ...
  _activateFullMode() {
    ...
    const centralOrch = tryRequire(path.join(__dirname, 'external-sense'), 'CentralOrchestrator');
    ...
    const saasOrch = tryRequire(path.join(__dirname, 'tenant-queue'), 'SaaSOrchestratorV4');
    if (saasOrch && typeof saasOrch.start === 'function') {
      try {
        const s = typeof saasOrch.getStatus === 'function' ? saasOrch.getStatus() : {};
        if (!s.running && !s.active) saasOrch.start();

...
'use strict';
/*
 * unicorn-enterprise-pipeline-activator
 * ----------
 * Unicorn Enterprise Pipeline Activator
 * Domain: sales-pipeline
 *
 * Activates enterprise pipeline (lead → qualification → proposal → close) using existing modules.
 *
...
const NAME   = 'unicorn-enterprise-pipeline-activator';
const TITLE  = 'Unicorn Enterprise Pipeline Activator';
...
function getStatus(opts) {
  return {
    ...
    title: TITLE,
    domain: DOMAIN,
    summary: `Activates enterprise pipeline (lead → qualification → proposal → close) using existing modules.`,

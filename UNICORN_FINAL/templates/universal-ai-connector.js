...
function loadKnownModels() {
  ...
  // --- PATCH:FREE-PROVIDERS ---
  // Helper: treat placeholder values as empty
  const _isPh = v => !v || /^your_.*_here$/i.test(v) || /^(skip|changeme|todo|placeholder)$/i.test(v) || v.length < 10;
  const _key  = k => { const v = process.env[k]; return _isPh(v) ? null : v; };

  // Mistral (chat)
  if (_key('MISTRAL_API_KEY')) {
    models.set('mistral-medium', {
      type: 'openai',
      endpoint: 'https://api.mistral.ai/v1/chat/completions',
      apiKey: _key('MISTRAL_API_KEY'),
      modelName: process.env.MISTRAL_MODEL || 'mistral-medium-latest',
      capabilities: ['text-generation', 'reasoning', 'code', 'chat', 'creative'],
      cost: 0.0001,
      performance: { speed: 0.90, accuracy: 0.92 },
    });
  }

  // Groq (Llama 3.3 70B, DeepSeek R1, Qwen - blazing fast)
  if (_key('GROQ_API_KEY')) {
    models.set('groq-llama-3.3-70b', {
      type: 'openai',
      endpoint: 'https://api.groq.com/openai/v1/chat/completions',
      apiKey: _key('GROQ_API_KEY'),
      modelName: process.env.GROQ_MODEL || 'llama-3.3-70b-versatile',
      capabilities: ['text-generation', 'reasoning', 'code', 'chat', 'creative'],
      cost: 0, // free tier
      performance: { speed: 0.99, accuracy: 0.93 }, // Groq is fastest
    });
    models.set('groq-deepseek-r1', {
      type: 'openai',
      endpoint: 'https://api.groq.com/openai/v1/chat/completions',
      apiKey: _key('GROQ_API_KEY'),
      ...
      capabilities: ['text-generation', 'reasoning', 'code'],
      cost: 0,
      performance: { speed: 0.97, accuracy: 0.96 },
    });
  }

  // OpenRouter (300+ models through one key; many free)
  if (_key('OPENROUTER_API_KEY')) {
    models.set('openrouter-llama-free', {
      type: 'openai',
      endpoint: 'https://openrouter.ai/api/v1/chat/completions',
      apiKey: _key('OPENROUTER_API_KEY'),
      modelName: process.env.OPENROUTER_MODEL || 'meta-llama/llama-3.3-70b-instruct:free',
      capabilities: ['text-generation', 'reasoning', 'code', 'chat', 'creative'],
      cost: 0,
      performance: { speed: 0.85, accuracy: 0.93 },
    });
    models.set('openrouter-deepseek-free', {
      type: 'openai',
      endpoint: 'https://openrouter.ai/api/v1/chat/completions',
      apiKey: _key('OPENROUTER_API_KEY'),
      ...
      capabilities: ['text-generation', 'reasoning', 'code'],
      cost: 0,
      performance: { speed: 0.80, accuracy: 0.95 },
    });
  }

  // Cerebras (world's fastest inference, 1800 tok/s)
  if (_key('CEREBRAS_API_KEY')) {
    models.set('cerebras-llama-3.3-70b', {
      type: 'openai',
      endpoint: 'https://api.cerebras.ai/v1/chat/completions',
      apiKey: _key('CEREBRAS_API_KEY'),
      modelName: process.env.CEREBRAS_MODEL || 'llama-3.3-70b',
      capabilities: ['text-generation', 'reasoning', 'code', 'chat', 'creative'],
      cost: 0,
      performance: { speed: 1.0, accuracy: 0.93 }, // ABSOLUTE FASTEST
    });
  }

  // Together AI
  if (_key('TOGETHER_API_KEY')) {
    models.set('together-llama-3.3', {
      type: 'openai',
      endpoint: 'https://api.together.xyz/v1/chat/completions',
      apiKey: _key('TOGETHER_API_KEY'),
      modelName: process.env.TOGETHER_MODEL || 'meta-llama/Llama-3.3-70B-Instruct-Turbo-Free',
      capabilities: ['text-generation', 'reasoning', 'code', 'chat', 'creative'],
      cost: 0,
  ...
  if (_key('CODESTRAL_API_KEY')) {
    models.set('codestral', {
      type: 'openai',
      endpoint: 'https://codestral.mistral.ai/v1/chat/completions',
      apiKey: _key('CODESTRAL_API_KEY'),
      modelName: process.env.CODESTRAL_MODEL || 'codestral-latest',
      capabilities: ['code', 'text-generation', 'reasoning'],
      cost: 0,
      performance: { speed: 0.90, accuracy: 0.97 }, // best code accuracy
    });
  }

  // HuggingFace with token (rate limits lifted)
  const _hfKey = _key('HUGGINGFACE_API_KEY') || _key('HF_API_KEY');
  if (_hfKey) {
    models.set('hf-qwen-coder', {
      type: 'openai',
      endpoint: 'https://api-inference.huggingface.co/v1/chat/completions',
      apiKey: _hfKey,
      modelName: process.env.HF_MODEL || 'Qwen/Qwen2.5-Coder-32B-Instruct',
      capabilities: ['code', 'text-generation', 'reasoning'],
      cost: 0,
      performance: { speed: 0.75, accuracy: 0.94 },
    });
  }

  // GitHub Models (GPT-4o, o1, Claude, Llama — free via GH_PAT)
  const _ghKey = _key('GH_PAT') || _key('GITHUB_TOKEN_SYNC') || _key('GITHUB_TOKEN');
  if (_ghKey) {
    models.set('github-gpt-4o', {
      type: 'openai',
      ...
      capabilities: ['text-generation', 'reasoning', 'code', 'chat', 'creative'],
      cost: 0,
      performance: { speed: 0.90, accuracy: 0.97 },
    });
    models.set('github-o1-mini', {
      type: 'openai',
      endpoint: 'https://models.inference.ai.azure.com/chat/completions',
      apiKey: _ghKey,
      modelName: 'o1-mini',
      capabilities: ['text-generation', 'reasoning', 'code'],
      cost: 0,
      performance: { speed: 0.70, accuracy: 0.99 },
    });
  }
  // --- END PATCH:FREE-PROVIDERS ---

...
check('server-rendered entitlement links contain the access token, not "${TOK}"', () => {
  ...
  // Preferred: /checkout/:id/qr.svg (nginx ^~ /checkout/ → site). Legacy /api/ path also OK.
  assert.ok(
    html.includes('src="/checkout/' + ORDER_ID + '/qr.svg"')
      || html.includes('src="/api/checkout/' + ORDER_ID + '/qr.svg"'),
    'checkout QR must use first-party site endpoint'
  );

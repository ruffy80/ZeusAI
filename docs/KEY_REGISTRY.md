...
```
TELEGRAM_BOT_TOKEN TELEGRAM_CHAT_ID  ZEUS_TG_GROUP_CHAT_ID
...
LINKEDIN_ACCESS_TOKEN LINKEDIN_AUTHOR_URN LINKEDIN_ORG_ID
...
Organic posting (autoviralizer) needs **page/user tokens**, not ads keys.
`TIKTOK_ADS_API_KEY` / `META_ADS_API_KEY` are paid-ads only and do not publish organic posts.
TikTok organic publish additionally requires a video file; without one the viralizer
